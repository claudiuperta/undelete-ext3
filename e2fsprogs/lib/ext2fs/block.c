 ...
 *
 * We iterate over the blocks of an inode. Just that instead of 
 * reading the inode, we first get it from the FIFO queue of 
 * deleted files and pass it as a parameter to the function. 
 */
errcode_t ext3u_block_iterate2(ext2_filsys fs, struct ext2_inode * inode, int flags, char *block_buf,
				int (*func)(ext2_filsys fs,
					    blk_t	*blocknr,
				...
				void *priv_data)
{
	...
	if (inode->i_block[EXT2_IND_BLOCK] || (flags & BLOCK_FLAG_APPEND)) {
		ret |= block_iterate_ind(&(inode->i_block[EXT2_IND_BLOCK]), 0, EXT2_IND_BLOCK, &ctx);
		if (ret & BLOCK_ABORT)
			goto abort_exit;
	} else
		ctx.bcount += limit;
	if (inode->i_block[EXT2_DIND_BLOCK] || (flags & BLOCK_FLAG_APPEND)) {
		ret |= block_iterate_dind(&(inode->i_block[EXT2_DIND_BLOCK]), 0, EXT2_DIND_BLOCK, &ctx);
		if (ret & BLOCK_ABORT)
			goto abort_exit;
	} else
		ctx.bcount += limit * limit;
	if (inode->i_block[EXT2_TIND_BLOCK] || (flags & BLOCK_FLAG_APPEND)) {
		ret |= block_iterate_tind(&(inode->i_block[EXT2_TIND_BLOCK]), 0, EXT2_TIND_BLOCK, &ctx);
		if (ret & BLOCK_ABORT)
			goto abort_exit;
	}
